...
class Componente2 extends Component {
              ...
              <tr className="meseInfo" style={{width:"100%"}}>
                <td style={{display:"flex",height:"calc(100% - 10px)",paddingLeft:"5px",paddingBottom:"5px",background: `linear-gradient(0deg,  #e0f1eb ${((elementi.importo*100/this.state.max))}%,#fff ${(elementi.importo*100/this.state.max)}%)`, borderBottom:`4px solid ${this.state.color[index]}`}}>
                  <div style={{alignSelf:"flex-end"}}>
                    <p className="doc">{this.formattaNumero(elementi.documenti)} doc.</p>
                    <p className="importo">{elementi.importo} €</p>
                  </div>
                </td>
              </tr>
            </tbody>
  ...
  mousePremi=(indice)=>{
    ...
    if(appoggioColori[indice]=="#0D97D5"){
      appoggioColori.fill("#0D97D5");
  ...
  entraMese=(indice)=>{
    if(this.azioneMouse=="mousePremi"){
      ...
      if(appoggioMesi[appoggioMesi.length-1]==(indice+this.verso)){
        appoggioColori[appoggioMesi[appoggioMesi.length-1]]="#0D97D5";
        appoggioMesi.pop();
      }else{
        appoggioColori[indice]="#f90";
        appoggioMesi.push(indice);
  ...
  stampaMesi=()=>{
    ...
    //In caso di selezione da destra a sinistra, l'ordine dei mesi nell'array è quello corretto e non al contrario
